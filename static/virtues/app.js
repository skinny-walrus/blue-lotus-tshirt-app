...
products.metta_cap={name:'Mettā Loving-kindness Cap',file:'loving-kindness-cap.png',cap:true};
products.karuna_cap={name:'Karuṇā Compassion Cap',file:'compassion-cap.png',cap:true};
const selections={...virtues,metta_cap:{...virtues.metta},karuna_cap:{...virtues.karuna},upekkha_koi:{...virtues.upekkha,name:'Equanimity – Koi'},karuna_crane:{...virtues.karuna,name:'Compassion · Crane'}};
...
let cart=[];try{const saved=JSON.parse(localStorage.getItem('blueLotusCart')||'[]');if(Array.isArray(saved))cart=saved.filter(x=>x&&typeof x.breed==='string'&&(Object.values(products).some(p=>p.cap&&p.name===x.breed)?x.color==='Stone'&&x.size==='One size fits all':Object.hasOwn(colors,x.color)&&sizes.includes(x.size))&&Number.isInteger(x.quantity)&&x.quantity>=1&&x.quantity<=10)}catch{}
function quantity(){return Number($('quantity').value)}
function priceFor(size){return size==='2XL'?3800:3500}
function update(){
 const cap=Boolean(products[state.virtue]?.cap);
 const v=selections[state.virtue],c=colors[state.color],front=state.side==='front';
 $('virtue').value=state.virtue;$('productName').textContent=v.name+(cap?' Cap':' Tee');$('previewTitle').textContent=v.name+' · '+(cap?'Stone':state.color);
 ...
 if(product&&!cap){artwork.src='/static/previews/'+product.file.replace(/\.png$/,'.webp');artwork.alt=product.name+' artwork and blessing'}
 ...
 $('colors').hidden=cap;$('sizes').hidden=cap;
 $('capColor').hidden=!cap;$('capSize').hidden=!cap;
 if(cap){$('placement').textContent='Front · '+v.name+' cap';$('price').textContent='$35.00';$('addButton').textContent='Add to cart · '+money(3500*(valid?quantity():1))}
...
for(const [id,parent,text] of [['capColor','colors','Stone'],['capSize','sizes','One size fits all']]){const label=document.createElement('p');label.id=id;label.textContent=text;label.hidden=true;$(parent).after(label)}
...
$('addButton').onclick=()=>{
 ...
 const item={breed:products[state.virtue].name,color:products[state.virtue].cap?'Stone':state.color,size:products[state.virtue].cap?'One size fits all':state.size,quantity:quantity(),unit_price_cents:products[state.virtue].cap?3500:priceFor(state.size)};
